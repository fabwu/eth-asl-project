% Package includes
\usepackage{amsthm}
\usepackage{amsmath}
\usepackage{spconf,amssymb,graphicx}
\usepackage[english]{babel}
\usepackage[utf8]{inputenc}

\usepackage{ifthen}
\usepackage{xifthen}

% Definitions
\newcommand{\R}[0]{\mathbb{R}}
\newcommand{\C}[0]{\mathbb{C}}
\newcommand{\mypar}[1]{{\bf #1.}}
\DeclareMathOperator*{\argmin}{arg\,min}

% Folder for images
\graphicspath{{images/}}

% set pdf attributes
\usepackage[pdftex,
pdftitle={Performance Optimized Fractal Image Compression with Quadtree Partitioning},
pdfauthor={Jonas Hansen, Pascal Huber, Fabian Wuethrich},
pdfkeywords={ETH, Fastcode, ASL, Fractal Image Compression, Quadtree}
]{hyperref}

\newtheorem{definition}{Definition}
\newtheorem{theorem}{Theorem}

% Inline notes with one color per person
\usepackage{todonotes}

\newcommand{\notepascal}[1]{
  \ifthenelse{\boolean{shownotes}}{
    \todo[inline,color=red!20]{\textbf{P:} #1}
  }{}
}
\newcommand{\notefabian}[1]{
  \ifthenelse{\boolean{shownotes}}{
    \todo[inline,color=green!20]{\textbf{F:} #1}
  }{}
}
\newcommand{\notejonas}[1]{
  \ifthenelse{\boolean{shownotes}}{
    \todo[inline,color=blue!20]{\textbf{J:} #1}
  }{}
}

% Algorithm stuff
\usepackage{algorithm}
\usepackage[noend]{algpseudocode}
\usepackage{algorithmicx}
\algnewcommand{\NULL}{\textsc{null}}
\algrenewcommand\textproc{\texttt}

% less margins around captions
% TODO: fix spacing after/before images/captions
% \setlength{\belowcaptionskip}{20pt}
% \addtolength{\textfloatsep}{-2.2in}
% \setlength{\intextsep}{10pt plus 2pt minus 2pt}

% don't highlight links
\usepackage{xcolor}
\hypersetup{
    colorlinks,
    linkcolor={red!50!black},
    citecolor={blue!50!black},
    urlcolor={blue!50!black}
}

% When referencing an image and clicking on the link, jump to the top of the
% image and not to the caption below the image.
\usepackage{hyperref}
\usepackage[all]{hypcap}

% for subfigures
\usepackage{caption}
\usepackage{subcaption}

% space efficient itemize
\usepackage{enumitem}

% Schusterjunge & Hurenkinder
\widowpenalty10000
\clubpenalty10000

% prevent too long lines
\usepackage{microtype}

\hyphenation{roof-line}
