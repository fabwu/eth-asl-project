...
algorithm where we assume that no transformation will yield an error below the
threshold and thus the only termination criteria is the maximum quadtree depth.
In practice, the cost will be (much) lower assuming a reasonable error threshold
$\epsilon$. As a cost metric, the number of floating point operations are
counted.
