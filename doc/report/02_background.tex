...
The following theorem about attractors has been proved by Hutchinson in
...
\mypar{Quadtree Parititioning} In practice, quadtree partitioning is considered
...
\mypar{Cost analysis} In this course we focus on floating point operations and ignore all 
...
The cost of our algorithm does not only depend on the size but also on the content
of an image. To classify our version of fractal image compression, we provide a 
pessimistic upper bound on the cost for our algorithm. We assume that no transformation 
will yield an error below the threshold and thus the only termination criteria is the 
maximum quadtree depth. In practice, the cost will be (much) lower assuming a reasonable 
...
$\sum_{i=1}^{n} b_i$,$\sum_{i=1}^{n} b_i^2$ are the same for all four rotations
and $\sum_{i=1}^{n} a_i b_i$ has to be computed for each rotation.
