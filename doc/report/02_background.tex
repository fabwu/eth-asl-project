...
\cite{fisher2012}.
\notejonas{Angepasst, bitte reviewen}
...
\begin{definition}[Iterated function system]
  ...
  \begin{align*}
    \{f_i:X \to X | i=1,...,n\}
  \end{align*}
  where each $f_i$ is contractive and $X$ is a metric space with metric $d$.
\end{definition}
...
$A$ is the image to be compressed, one only needs to store the transformations. For
