...
A simple approach is exhaustive block mapping, where each range block is compared with each domain block.\\
...
denote the cost to compute four transformations $w_i^{0^\circ}$, $w_i^{90^\circ}$, 
$w_i^{180^\circ}$, $w_i^{270^\circ}$  and their errors. Each transformation $w_i^{\gamma^\circ}$ 
