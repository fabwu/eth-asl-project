    ...
    while (has_remaining_range_blocks) {
        ...
        for (size_t idx_rb = 0; idx_rb < range_blocks_length_current_iteration;
            ...
            for (size_t idx_db = 0; idx_db < domain_blocks_length; ++idx_db) {
                ...
                double rtd_0 = 0;
                if (dbs >= 4 && dbs % 4 == 0) {
                    __m256d v_rtd_sum = _mm256_setzero_pd();
                    for (int i = 0; i < dbs; ++i) {
                        for (int j = 0; j < dbs; j += 4) {
                            __m256d v_ri =
                                _mm256_loadu_pd(image->data + rtd_start_rb +
                                                i * image->size + j);
                            __m256d v_di = _mm256_loadu_pd(
                                downsampled_db->data + i * dbs + j);

                            v_rtd_sum = _mm256_fmadd_pd(v_ri, v_di, v_rtd_sum);
                        }
                    }
                    rtd_0 = v_rtd_sum[0] + v_rtd_sum[1] + v_rtd_sum[2] +
                            v_rtd_sum[3];
                    __record_double_flops(dbs * dbs * 0.5 +
                                          3);  // todo correcto?
                } else {
                    for (int i = 0; i < dbs; i++) {
                        for (int j = 0; j < dbs; ++j) {
                            double ri =
                                image->data[rtd_start_rb + i * image->size + j];
                            double di = downsampled_db->data[i * dbs + j];
                            rtd_0 = fma(ri, di, rtd_0);
                        }
                    }
                ...
                __record_double_flops(dbs * dbs * 2 + 1);
