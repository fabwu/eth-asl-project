...
pair<double, double> solve_2x2_eq_system(double a11, double a12, double a21,
...
pair<double, double> compute_brightness_and_contrast(const image_t &image,
                                                     double *domain,
...
inline double diff_block(const image_t &image,
                         const image_t *scaled_source_block,
                         block_t target_block, int contrast, int brightness) {
  int n = scaled_source_block->size;
  double squared_error = 0.0;
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < n; ++j) {
      double target_val =
          image.data[target_block.get_index_in_image(i, j, image.size)];
      double source_val =
          scaled_source_block->data[i * n + j] * contrast + brightness;
  ...
  for (const block_t target_block : target_blocks) {
    ...
    for (int source_block_index = 0; source_block_index < source_blocks.size();
      ...
      for (auto angle : all_angles) {
        ...
        std::tie(brightness, contrast) = compute_brightness_and_contrast(
            image, rotated_source_block.data, target_block);

        double target_block_error = diff_block(
            image, &rotated_source_block, target_block, contrast, brightness);
