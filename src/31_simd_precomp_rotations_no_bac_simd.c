    ...
    for (int current_quadtree_depth = MIN_QUADTREE_DEPTH;
        ...
        for (size_t idx_rb = 0; idx_rb < range_blocks_length_current_iteration;
            ...
            for (size_t idx_db = 0; idx_db < domain_blocks_length; ++idx_db) {
                ...
                double contrast, brightness, error;
                {
                    contrast = fma(num_pixels, rtd, -sd_x_sr) / denominator;
                    brightness = fma(contrast, -domain_sum, range_sum) *
                                 num_pixels_of_blocks_inv;
                    double a1 = fma(num_pixels, brightness, -sr_x_2);
                    double a2 = fma(brightness, a1, range_sum_squared);
                    double a3 = fma(brightness, sd_x_2, -rtd);
                    double a4 = fma(contrast, domain_sum_squared, -rtd);
                    double a5 = a3 + a4;
                    error = fma(a5, contrast, a2);
            ...
            for (size_t idx_db = 0; idx_db < domain_blocks_length; ++idx_db) {
                ...
                double contrast, brightness, error;
                {
                    ...
                    double a4 = fma(contrast, domain_sum_squared, -rtd);
                    double a5 = a3 + a4;
                    error = fma(a5, contrast, a2) * num_pixels_of_blocks_inv;
                }
                __record_double_flops(18);
