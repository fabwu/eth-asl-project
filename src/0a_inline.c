...
struct queue *compress(const struct image_t *image, const int error_threshold) {
    ...
    int db_size_current_iteration = -1;
    int num_pixels;
    ...
    *transformations = make_queue();
    ...
    while (has_remaining_range_blocks) {
        ...
        {
            num_of_rb = range_blocks_length_next_iteration;
        ...
        // BEGIN Prepare the domain blocks

        db_size_current_iteration = 2 * range_blocks_size_current_iteration;
        assert(db_size_current_iteration >= 2);
        domain_blocks =
            create_squared_blocks(image->size, db_size_current_iteration, 0, 0);
        int num_of_db = (image->size / db_size_current_iteration) *
                        (image->size / db_size_current_iteration);
        int db_size = range_blocks_size_current_iteration;
        int db_size_sqr = db_size * db_size;
        
        double *db_data = (double *)malloc(num_of_db * db_size_sqr * sizeof(double));
        double *db_sums = (double *)malloc(sizeof(double) * num_of_db);
        double *db_sums_squared = (double *)malloc(sizeof(double) * num_of_db);

        // scale domain blocks
        for (size_t i = 0; i < num_of_db; ++i) {
            struct block_t *block = domain_blocks + i;
            double *scaled_db = db_data + i * db_size_sqr;

            size_t scaled_image_idx = 0;
            size_t original_image_idx =
                block->rel_y * image->size + block->rel_x;
            for (int y = 0; y < block->height; y += 2) {
                for (int x = 0; x < block->width; x += 2) {
                    double val = 0.0;
                    val += image->data[original_image_idx];
                    ...
                    val += image->data[original_image_idx + image->size];
                    val += image->data[original_image_idx + image->size + 1];
                    scaled_db[scaled_image_idx] = val * 0.25;
                    scaled_image_idx++;
                    original_image_idx += 2;
            ...
            for (size_t j = 0; j < db_size_sqr; j++) {
                sum += scaled_db[j];
                sum_squared += scaled_db[j] * scaled_db[j];
            }
            __record_double_flops(db_size_sqr * 3);

            db_sums[i] = sum;
            db_sums_squared[i] = sum_squared;
        }
        __record_double_flops(4);
        // END Prepare domain blocks

        // BEGIN Prepare range blocks
        double *rb_sums = (double *)malloc(sizeof(double) * num_of_rb);
        double *rb_sums_squared = (double *)malloc(sizeof(double) * num_of_rb);
        for (int i = 0; i < num_of_rb; ++i) {
            const struct block_t *rb = range_blocks_curr_iteration + i;
            double sum = 0;
            double sum_squared = 0;
            int ind = rb->rel_y * image->size + rb->rel_x;
            for (int l = 0; l < rb->width; l++) {
                for (int m = 0; m < rb->height; m++) {
                    sum += image->data[ind];
            ...
            rb_sums[i] = sum;
            rb_sums_squared[i] = sum_squared;
            __record_double_flops(rb->width * rb->height * 3);
        }
        //END Prepare range blocks

        // Process each range block
        for (size_t idx_rb = 0; idx_rb < num_of_rb; ++idx_rb) {
            ...
            const double range_sum = rb_sums[idx_rb];
            const double range_sum_squared = rb_sums_squared[idx_rb];
            const double sr_x_2 = range_sum + range_sum;
            __record_double_flops(1);

            int rtd_start_rb =
                range_block->rel_y * image->size + range_block->rel_x;

            for (size_t idx_db = 0; idx_db < num_of_db; ++idx_db) {
                assert(domain_blocks[idx_db].width == 2 * range_block->width);
                double *current_db = db_data + idx_db * db_size_sqr;

                const double domain_sum = db_sums[idx_db];
                const double domain_sum_squared =
                    db_sums_squared[idx_db];
                ...
                for (int i = 0; i < db_size; i++) {
                    int dbs_j = 0;
                    for (int j = 0; j < db_size; j += 2) {
                        int idx_0_db1 = dbs_i + j;
                        int idx_0_db2 = idx_0_db1 + 1;
                        int idx_90_db1 = db_size_sqr - dbs_j - db_size + i;
                        int idx_90_db2 = idx_90_db1 - db_size;
                        int idx_180_db1 = db_size_sqr - dbs_i - j - 1;
                        int idx_180_db2 = idx_180_db1 - 1;
                        int idx_270_db1 = dbs_j + db_size - i - 1;
                        int idx_270_db2 = idx_270_db1 + db_size;
                        ...
                        double di_0_1 = current_db[idx_0_db1];
                        double di_0_2 = current_db[idx_0_db2];
                        double di_90_1 = current_db[idx_90_db1];
                        double di_90_2 = current_db[idx_90_db2];
                        double di_180_1 = current_db[idx_180_db1];
                        double di_180_2 = current_db[idx_180_db2];
                        double di_270_1 = current_db[idx_270_db1];
                        double di_270_2 = current_db[idx_270_db2];
                        ...
                        rtd_idx_rb += 2;
                        dbs_j = dbs_j + db_size + db_size;
                    }
                    rtd_idx_rb += image->size - db_size;
                    dbs_i += db_size;
                }

                __record_double_flops(db_size * db_size * 8);
        ...
        free(rb_sums);
        free(rb_sums_squared);
        free(db_sums);
        free(db_sums_squared);
        free(domain_blocks);
        free(db_data);
    }
